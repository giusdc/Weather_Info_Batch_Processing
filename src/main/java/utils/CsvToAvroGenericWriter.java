...
public class CsvToAvroGenericWriter
{
	...
	private static Schema NULL_SCHEMA 				= Schema.create(Type.NULL);
    ...
    public void append(String[] fields) throws Exception
    {
    	...
    	if(nullFields.size()>0)
    	{
    		throw new Exception("fields with null values but the schema does not allow for null: " + nullFields);
    	}
    	else
    	{
    		dataFileWriter.append(record);
    ...
    {
    	return line.split(separatorCharacter,-1);
    }
